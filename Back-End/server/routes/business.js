...
const authenticateToken = async (req, res, next) => {
  try {
    ...
    if (decoded.adminId) {
      console.log('🔍 Backend: Admin token detected');
      ...
      req.user = admin;
      req.user.userType = 'admin'; // Add userType for consistency
      console.log('🔍 Backend: Admin authenticated:', admin.username);
    } else if (decoded.userId) {
      console.log('🔍 Backend: User token detected');
...
router.post('/verification-docs', authenticateToken, async (req, res) => {
  console.log('🔍 Verification docs endpoint hit:', req.originalUrl);
  console.log('📝 Request body:', req.body);
  console.log('🔑 User ID:', req.user._id);
  
  try {
    const { businessId, license, governmentId, supporting = [] } = req.body || {};
    console.log('🔍 Parsed data:', { businessId, hasLicense: !!license, hasGovernmentId: !!governmentId, supportingCount: supporting.length });
    ...
    console.log('🔍 Found business:', { 
      ...
      currentDocs: business.verification?.documents?.length || 0 
    });
    ...
    console.log('🔍 Adding documents:', { docsCount: docs.length, existingDocs: business.verification.documents.length });
    
    business.verification.documents = [...(business.verification.documents || []), ...docs];
    
    console.log('🔍 After adding documents:', { totalDocs: business.verification.documents.length });
    
    // Keep business active, don't change status to pending
    await business.save();
    
    console.log('✅ Business verification docs saved successfully');
    console.log('🔍 Final business state:', { 
      id: business._id, 
      docsCount: business.verification.documents.length,
...
router.get('/verification/pending', authenticateToken, async (req, res) => {
  try {
    console.log('🔍 Backend: Verification/pending endpoint called');
    ...
    console.log('✅ Backend: User is admin, proceeding...');
    ...
    console.log('🔍 Backend: All businesses:', allBusinesses.map(b => ({
      name: b.businessName,
    ...
    console.log('🔍 Backend: Found pending verifications:', items.length);
    ...
    console.log('🔍 Filtered results:', {
      ...
      businessesWithDocs: businessesWithDocs.map(b => ({ 
        name: b.businessName, 
        docsCount: b.verification?.documents?.length || 0,
    ...
    console.error('❌ Backend: Error fetching pending verifications:', e);
...
router.get('/', async (req, res) => {
  try {
    ...
    console.log('🔍 Backend: Cache miss, querying database...');
    ...
    if (search && search.trim()) {
      ...
      try {
        ...
        console.log('🚀 Backend: Text search completed, serving response');
    ...
    console.log('🔍 Backend: Executing optimized query...');
    ...
    console.log('🔍 Backend: Found businesses:', businesses.length);
    ...
    console.log('🚀 Backend: Query completed, serving response');
    res.json(response);
...
router.post('/reset-verification', authenticateToken, async (req, res) => {
  try {
    ...
    console.log('🔍 Backend: Reset verification for', result.modifiedCount, 'businesses');
    res.json({ 
      message: `Reset verification for ${result.modifiedCount} businesses`,
      modifiedCount: result.modifiedCount
    });
  } catch (e) {
    console.error('❌ Backend: Error resetting verification:', e);
...
router.get('/debug/all', async (req, res) => {
  try {
    ...
    console.log('🔍 Debug: All businesses:', businesses.map(b => ({
      name: b.businessName,
      verified: b.verification?.isVerified,
